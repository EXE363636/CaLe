...
- **Contextual help must be click/tap accessible.** *(Phase 9Y-Fix rule)* Every `(?)` glyph must open a real popover/modal, not a hover-only tooltip. Mobile users have no hover state. Use `<HelpPopover>` from `src/components/ui/HelpPopover.tsx`. Title comes from the surrounding label, description is a 1–3 sentence concept blurb, optional `learnMoreHref` defaults to `/user-guide`. ESC + outside click + close button all dismiss (inherited from `<Modal>`).
