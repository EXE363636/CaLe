        ...
        <div className="ml-auto flex min-w-0 shrink-0 items-center gap-1 xl:ml-0 xl:justify-self-end">
          ...
          {role === null && (
            <div className="hidden items-center gap-1.5 xl:flex">
              ...
              {/* Primary employer CTA — gradient with subtle shimmer on hover */}
              <Link
                href="/register?role=employer"
                className="ml-1 inline-flex min-h-[40px] items-center whitespace-nowrap rounded-lg bg-gradient-to-br from-orange-500 via-orange-500 to-orange-600 px-4 text-sm font-semibold text-white shadow-md ring-1 ring-orange-600/20 transition-all duration-200 hover:shadow-[0_4px_16px_rgba(249,115,22,0.35)] hover:-translate-y-0.5 focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-400 focus-visible:ring-offset-1"
              >
                Đăng ca tuyển
              </Link>
            </div>
...
function navLinkClasses(active: boolean): string {
  return [
    // Compact horizontal padding at `xl`, roomier at `2xl`.
    // min-h-[40px] ensures 44px touch target when vertically centered.
    'whitespace-nowrap rounded-lg px-2 py-2 text-sm font-medium min-h-[40px] flex items-center 2xl:px-3',
    'transition-all duration-150 ease-out',
    'focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-400',
    active
      ? 'bg-orange-500/10 text-orange-600 [data-theme=dark]:bg-orange-500/15 [data-theme=dark]:text-orange-400'
    ...
    >
      ...
      {isOpen && (
        <div
          ...
          className="modal-panel-anim absolute left-0 top-full z-40 mt-2 w-76 min-w-[17rem] rounded-2xl p-2 glass-panel"
          style={{ boxShadow: 'var(--shadow-brand), var(--shadow-lg)' }}
        >
          {/* Subtle gradient header line */}
          <div className="mb-1.5 px-3 pt-1 pb-2 border-b border-orange-100/60" style={{ borderColor: 'var(--border)' }}>
            ...
            {group.items.map((item) => (
              <li key={item.href} role="none">
                <Link
                  ...
                  role="menuitem"
                  onClick={() => onItemClick()}
                  className="group flex flex-col rounded-xl px-3 py-2.5 text-sm transition-all duration-150 hover:bg-orange-50/80 focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-400"
                  style={{
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    ['--tw-ring-offset-color' as any]: 'var(--surface)',
                  }}
                >
                  ...
                  {item.description && (
                    <span className="mt-0.5 text-xs" style={{ color: 'var(--muted)' }}>
                      {item.description}
                    </span>
                  )}
                </Link>
