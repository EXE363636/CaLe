...
interface ApplicationStore {
  ...
  /** Vắng mặt → có mặt (đến muộn). Supabase: RPC employer_revert_no_show. */
  revertNoShowToPresentAsync(
    applicationId: string,
    reason: string,
...
export const useApplicationStore = create<ApplicationStore>((set, get) => ({
  ...
  async revertNoShowToPresentAsync(applicationId, reason) {
    const app = get().applications.find((a) => a.id === applicationId);
    if (getDataMode() === 'supabase') {
      try {
        await getApplicationRepo().employerRevertNoShow(applicationId, reason);
        if (app) {
          await get().refetchForShift(app.shiftId);
          await useShiftStore.getState().refetchOne(app.shiftId);
        }
        ...
        return { ok: false, error: e instanceof Error ? e.message : 'REVERT_FAILED' };
      }
    }
    const r = get().revertNoShowToPresent(applicationId, reason);
    return r.ok ? { ok: true, value: undefined } : { ok: false, error: r.error };
  },
